...
import { Activity, Clock, Award, ShieldAlert, ArrowRight, CheckCircle, RefreshCw, Zap, Calendar, CheckSquare } from 'lucide-react';
import Link from 'next/link';
import { motion } from 'framer-motion';
import { MyPerformanceWidget } from '../../../components/MyPerformanceWidget';

export default function OPTechnicianDashboard() {
  const { user } = useAuth();
  ...
  const pendingOPVerification = sessions.filter(s => !s.op_verified && s.status !== 'Cancelled' && s.status !== 'Completed').length;
  const verifiedOP = sessions.filter(s => s.op_verified && s.status !== 'Cancelled').length;
  ...
  const getFilteredSessions = () => {
    switch (activeListTab) {
      case 'pending':
        return sessions.filter(s => !s.op_verified);
      case 'verified':
        return sessions.filter(s => s.op_verified);
      case 'history':
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 className="text-lg sm:text-2xl font-bold text-slate-500 flex items-center gap-2">
              OP Technician Portal
              <Zap className="h-6 w-6 text-primary-green animate-pulse" />
            </h1>
            <p className="text-sm text-slate-500 mt-1">
              Welcome, <span className="font-semibold text-primary-green">{user?.name}</span>. Manage assigned sessions, log timings, and sign off verification records.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div style={{ background: 'white', border: '1px solid #E2E8F0' }} className="rounded-2xl p-5 relative overflow-hidden group shadow-sm">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-[10px] text-slate-450 mt-2">Assigned overall</p>
              </div>
          ...
          <Link href="/op-technician/therapies" style={{ background: 'white', border: '1px solid #E2E8F0' }} className="rounded-2xl p-5 relative overflow-hidden group shadow-sm hover:border-rose-350 transition-all cursor-pointer">
            <div className="absolute top-0 right-0 w-24 h-24 bg-rose-500/5 rounded-full blur-2xl pointer-events-none" />
            <div className="flex justify-between items-start">
              <div>
                <p style={{ color: '#475569', fontWeight: 600 }} className="text-xs uppercase tracking-wider">Pending Log / Verify</p>
                <h3 style={{ fontSize: '36px', fontWeight: 700, color: '#DC2626' }} className="mt-2 leading-none">{pendingOPVerification}</h3>
          ...
          <div style={{ background: 'white', border: '1px solid #E2E8F0' }} className="rounded-2xl p-5 relative overflow-hidden group shadow-sm">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 style={{ fontSize: '36px', fontWeight: 700, color: '#059669' }} className="mt-2 leading-none">{verifiedOP}</h3>
                <p className="text-[10px] text-emerald-600/70 mt-2">Signed by you</p>
              </div>
              <div className="p-3 bg-very-light-green rounded-xl border border-light-green text-primary-green">
          ...
          <div style={{ background: 'white', border: '1px solid #E2E8F0' }} className="rounded-2xl p-5 relative overflow-hidden group shadow-sm">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-[10px] text-slate-450 mt-2">Completed sessions</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-border-gray bg-slate-50/50 rounded-xl border border-border-gray p-2">
              {loading ? (
                ...
                getFilteredSessions().map((session) => (
                  <Link 
                    href={`/op-technician/therapies?tab=${encodeURIComponent(session.therapy_type)}`}
                    key={session.id} 
