...
import { 
  ...
  Settings, CheckSquare, X, Calendar, Plus, Droplet, MessageSquare
} from 'lucide-react';
...
function OPTherapiesContent() {
  ...
  const handleOpenVerify = (session: any) => {
    ...
    setOpTechnicianId(session.op_technician_id ? String(session.op_technician_id) : String(user?.id || ''));
    setSopTechnicianId(session.sop_technician_id ? String(session.sop_technician_id) : '');
    setOpVerified(session.op_verified === true || session.op_verified === 'true');
  ...
  const handleOpenCreate = () => {
    ...
    setOpTechnicianId(user?.id ? String(user.id) : '');
    setSopTechnicianId('');
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.therapies.verify(selectedSession.id, { verified: opVerified });

      setSuccess('Therapy session logs updated and signed successfully.');
  ...
  return (
    <div className="space-y-6 max-w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-lg sm:text-2xl font-bold text-slate-500 flex items-center gap-2">
            Therapy Workstations
            <Activity className="h-5 w-5 text-primary-green" />
          </h1>
          <p className="text-sm text-slate-400 mt-0.5">
            Log session logs, trace compliance and schedule therapy sessions.
          </p>
        </div>
      ...
      {/* Log Details & Verification Modal */}
      <AnimatePresence>
              ...
              <form onSubmit={handleFormSubmit} className="flex flex-col max-h-[90vh] w-full">
                ...
                <div className="px-6 py-4 border-b border-border-gray flex items-center justify-between shrink-0 bg-white">
                  <h3 className="font-bold text-sm text-slate-500 flex items-center gap-1.5">
                    <Settings className="h-4.5 w-4.5 text-primary-green animate-spin" />
                    Log Session & Verify: {selectedSession.patient_name}
                  </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {selectedSession.therapy_type === 'Lab' ? (
                      ...
                      <>
                        {/* Timings */}
                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Expected Timings Slot</label>
                          <input
                            type="text"
                            required
                            value={timings}
                        ...
                        {selectedSession.therapy_type === 'HBOT' && (
                          <div className="col-span-1 md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 border border-border-gray p-4 rounded-xl bg-slate-50/50">
                            <div>
                        ...
                        {selectedSession.therapy_type === 'Dental' && (
                          <div>
                            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Treatment routine type</label>
                            <input
                              type="text"
                              value={treatmentType}
                        ...
                        <div>
                          ...
                          <SelectField
                            ...
                            triggerClassName="py-2.5 px-3 text-xs text-slate-700"
                            options={[
                              { value: 'No', label: 'No' },
                              { value: 'Yes', label: 'Yes' }
                            ]}
                        ...
                        {/* Rescheduled details conditional */}
                        {rescheduled === 'Yes' && (
                          <div className="col-span-1 grid grid-cols-2 gap-2">
                            <div>
                              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">New Date</label>
                              <input
                                type="date"
                                required
                            ...
                            <div>
                              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">New Time</label>
                              <input
                                type="text"
                                required
                        ...
                        {/* Pressure / Oxygen Selector (HBOT & Ozone only, or custom name for Dental/Physiotherapy) */}
                        {selectedSession.therapy_type === 'HBOT' && (
                          <div className="col-span-1 md:col-span-2 border border-border-gray p-4 rounded-xl space-y-3 bg-slate-50/50">
                            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">Oxygen Level Selector</label>
                            <div className="flex gap-4">
                              <button
                                type="button"
                                onClick={() => setPressureType('Cylinder Pressure')}
                                className={`flex-1 py-2 px-3 rounded-lg text-xs font-semibold border transition-all cursor-pointer ${
                                  pressureType === 'Cylinder Pressure'
                                    ? 'bg-primary-green text-white border-primary-green shadow-sm'
                              ...
                              <button
                                type="button"
                                onClick={() => setPressureType('Tank Pressure')}
                                className={`flex-1 py-2 px-3 rounded-lg text-xs font-semibold border transition-all cursor-pointer ${
                                  pressureType === 'Tank Pressure'
                                    ? 'bg-primary-green text-white border-primary-green shadow-sm'
                                    : 'bg-white text-slate-500 border-border-gray hover:bg-slate-50'
                                }`}
                              >
                            ...
                            <div>
                              <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Enter Value (PSI)</label>
                              <div className="relative">
                                <Droplet className="absolute left-3 top-2.5 h-4 w-4 text-primary-green" />
                                <input
                                  type="number"
                                  required
                                  value={pressureValue}
                                  ...
                                  placeholder="e.g. 120"
                                  className="w-full bg-white border border-border-gray focus:border-primary-green rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-550 outline-none"
                                />
                              </div>
                            </div>
                          </div>
                        ...
                        {selectedSession.therapy_type === 'Ozone' && (
                          <div className="col-span-1 md:col-span-2 border border-border-gray p-4 rounded-xl space-y-4 bg-slate-50/50 text-xs">
                            <h4 className="font-bold text-[10px] text-slate-400 uppercase tracking-wider">Ozone Concentration Details</h4>
                        ...
                        {selectedSession.therapy_type === 'Physiotherapy' && (
                          <div className="col-span-1 md:col-span-2 border border-border-gray p-4 rounded-xl space-y-4 bg-slate-50/50 text-xs">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                              <div>
                                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Physiotherapist assigned</label>
                                <input
                                  type="text"
                              ...
                              <div>
                                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Select Routines / Therapies</label>
                              ...
                              <div className="flex items-end gap-2">
                                <div className="flex-1">
                                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Add Custom Therapy / Option</label>
                                  <input
                                    type="text"
                        ...
                        {selectedSession.therapy_type === 'Dental' && (
                          <div>
                            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Dentist Name</label>
                            <input
                              type="text"
                              value={dentistName}
                        ...
                        {/* Next session timings (Except hydrogen) */}
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Select OP Technician</label>
                      <SelectField
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Select SOP Technician</label>
                      <SelectField
                    ...
                    <div className="col-span-1 md:col-span-2 bg-emerald-50/50 border border-emerald-250 p-4 rounded-xl flex items-center justify-between gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-bold text-slate-700">OP Technician Verification Sign-off</p>
                          <p className="text-[10px] text-slate-400">Confirm you performed the session checks and log details correctly.</p>
                        </div>
                      </div>
                      <input
                        type="checkbox"
                        checked={opVerified}
                        onChange={(e) => setOpVerified(e.target.checked)}
                ...
                <div className="px-6 py-4 border-t border-border-gray flex items-center justify-end gap-2.5 shrink-0 bg-white">
                  ...
                  >
                    {submitLoading ? 'Saving...' : 'Save Logs & Sign'}
                  </button>
                </div>
              </form>
            </motion.div>
                      ...
                      <>
                        ...
                        <div className="col-span-1 md:col-span-2">
                          ...
                          <SelectField
                            ...
                            triggerClassName="py-2.5 px-3 text-xs font-semibold text-slate-755"
                            options={[
...
export default function OPTechnicianTherapies() {
  return (
    <DashboardLayout>
      <Suspense fallback={
